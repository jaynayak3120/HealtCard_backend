...
userRoute.get('/:id', async (req,res) => {
    try {
        const resp = await client.query('SELECT * FROM "Patient" WHERE "patientID"=$1',[req.params.id])
        delete resp.rows[0].patientPass
...
userRoute.get('/:patientID/cases', async (req, res) => {
    if(req.params.patientID === req.user._id){   
        try {
            const resp = await client.query('SELECT "Cases".*, "Prescription"."prescDetail", "Prescription"."dateOfPrescription", "Reports"."reportURL", "Reports"."dateOfReport", "Doctor"."docName" FROM "Cases" INNER JOIN "Patient" ON ("Cases"."patientID" = "Patient"."patientID") LEFT JOIN "Reports" ON ("Cases"."reportID" = "Reports"."reportID") LEFT JOIN "Prescription" ON ("Cases"."prescriptionID" = "Prescription"."prescriptionID") LEFT JOIN "Doctor" ON ("Cases"."docID" = "Doctor"."doctorID") where "Patient"."patientID"=$1',[req.user._id])
